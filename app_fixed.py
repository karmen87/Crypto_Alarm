"""
Crypto Price Alarm - Flask Application (Fixed for WebSocket timeout)
"""
...
def handle_add_asset(data):
    """Add a new trading pair to monitor - runs asynchronously"""
    pair = data.get('pair', '').strip().upper()
    print(f"📥 Request to add: {pair}")

    def add_asset_async():
        """Run the add_asset operation in a separate thread"""
        try:
            result = monitor.add_asset(pair)
            print(f"📤 Add asset result: {result.get('success')}")
    ...
    emit('asset_adding', {'pair': pair, 'message': 'Fetching data from CoinGecko... (this may take 5-10 seconds)'})
...
def handle_restart_alarm(data):
    ...
    try:
        ...
        print(f"❌ Error restarting alarm: {e}")
        emit('error', {'message': str(e)})
...
if __name__ == '__main__':
    print("=" * 60)
    print("🚀 Crypto Price Alarm Server Starting...")
    print("=" * 60)
    print("📍 Open http://localhost:5000 in your browser")
    print("⏱️  Adding pairs takes 5-10 seconds (API rate limiting)")
    print("⚠️  Wait at least 10 seconds between adding pairs")
    print("=" * 60)
